...
- (instancetype)init
{
    self = [super init];
    if (self) {
        UIBarButtonItem *backItem = [[UIBarButtonItem alloc]initWithTitle:@"返回" style:UIBarButtonItemStylePlain target:self action:@selector(popViewControllerAnimated:)];
...
- (void)viewWillLayoutSubviews{
    ...
    CGFloat top = 44;
    if (iOS7Later) top += 20;
    _tableView.frame = CGRectMake(0, top, self.view.tz_width, self.view.tz_height - top);
}
