...
- (NSString *)getBytesFromDataLength:(NSInteger)dataLength {
    ...
    if (dataLength >= 0.1 * (1024 * 1024)) {
        bytes = [NSString stringWithFormat:@"%0.1fMB",dataLength/1024/1024.0];
...
//            CGImageRef originalImageRef = [assetRep fullResolutionImage];
            CGImageRef originalImageRef = [assetRep fullResolutionImage];
            UIImage *originalImage = [UIImage imageWithCGImage:originalImageRef scale:1.0 orientation:(UIImageOrientation)assetRep.orientation];
...
- (UIImage *)scaleImage:(UIImage*)image toMaxSize:(CGSize)max{
    CGSize cur = image.size;
    int h = cur.height*image.scale;
    int w = cur.width*image.scale;
    if(w <= max.width
       && h <= max.height){
        return image;
    }else{
        float b = max.width/w < max.height/h ? max.width/w : max.height/h;
        UIImage *img = [self scaleImage:image toSize:CGSizeMake(b*w*image.scale, b*h*image.scale)];
        return img;
    }
}
